...
# Ensure text is preserved as text in SVG files
mpl.rcParams['svg.fonttype'] = 'none'

import matplotlib.pyplot as plt
# Optionally, you can explicitly switch to a Qt backend if it's not already the default:
...
Bmal_Data = pd.read_csv("./MergedData/Bmal1_equallength.csv", sep=";", index_col="Time")
Per2_Data = pd.read_csv("./MergedData/Per2_equallength.csv", sep=";", index_col="Time")

# Extract time information from index
t = Bmal_Data.index.to_numpy()
...
###
# Generate list of corresponding cell line names based on column names
###

# Bmal1
Bmal_Columns = Bmal_Data.columns.to_list()
Bmal_Celline = np.array([i.split(sep="_")[1] for i in Bmal_Columns])

# Per2
Per2_Columns = Per2_Data.columns.to_list()
Per2_Celline = np.array([i.split(sep="_")[1] for i in Per2_Columns])

#print(unique(Bmal_Celline, return_counts=True))
#print(unique(Per2_Celline, return_counts=True))

Celllines = np.unique( np.unique(Bmal_Celline).tolist() + np.unique(Per2_Celline).tolist() )
#print(Celllines, len(Celllines))
...
for ReporterDependentData, ReporterDependentCellines, c_rep in zip([Bmal_Data, Per2_Data], [Bmal_Celline, Per2_Celline], [BmalRep, Per2Rep]):
    ...
    for i, cellline in enumerate(Celllines):
        ...
        if len(index_of_cellline) != 0:
            ...
            axis("off")
    tight_layout()
    savefig("./Results/Plots/Raw_"+c_rep+".svg", dpi=450)
...
# Save detrended data in separate data frames
Bmal_DetrData = Bmal_Data.iloc[skip:]
Per2_DetrData = Per2_Data.iloc[skip:]
    ...
    for i, cellline in enumerate(Celllines):
        ...
        if len(index_of_cellline) != 0:
            ...
            for j, m in enumerate(Subset.columns.to_list()):
                #print(j, m)
                t_tmp = t[skip:]
                signal = Subset[m].values[skip:]
                trend = sinc_smooth(signal, T_cut_off, dt)
                detr_signal = signal - trend    # one could also think about dividing by the trend
                #detr_signal = signal / trend    # one could also think about dividing by the trend
                # save detrended data in data frames
                if c_rep == BmalRep:
                    #Bmal_DetrData[m] = detr_signal[:]
                    Bmal_DetrData.loc[:,m] = detr_signal
                elif c_rep == Per2Rep:
                    #Per2_DetrData[m] = detr_signal[:]
                    Per2_DetrData.loc[:,m] = detr_signal
                # sampling interval
                plot(t_tmp, detr_signal, label=m)
                #if c_rep == BmalRep:
                    #plot(t_tmp, Bmal_DetrData[m], label=m)
                #elif c_rep == Per2Rep:
                    #plot(t_tmp, Per2_DetrData[m], label=m)
            legend(loc=0, prop={"size":5})
            title(cellline)
        else:
            axis("off")
    tight_layout()
    savefig("Results/Plots/Detrended_"+c_rep+".svg", dpi=450)


Bmal_DetrData.to_csv("./Results/Data/Detrended_Bmal.csv", sep=";")
Per2_DetrData.to_csv("./Results/Data/Detrended_Per2.csv", sep=";")
