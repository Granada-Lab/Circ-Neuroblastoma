...
def benjamini_hochberg(p_values):
    ...
    # Ensure the adjusted p-values are not greater than 1
...
def find_ranks_in_bh_order(df, positions_dict, IS_DRUG_KEY=True):
    # Flatten the DataFrame and store the original positions
    p_values_flat = df.values.flatten()
    original_positions = [(i, j) for i in range(df.shape[0]) for j in range(df.shape[1])]
    # Create a DataFrame with p-values and their original positions
    p_values_df = pd.DataFrame({
        'p_value': p_values_flat,
        'original_position': original_positions})
    # Sort the DataFrame by p-value
    p_values_df.sort_values(by='p_value', inplace=True, ignore_index=True)
    # Assign ranks
    p_values_df['rank'] = p_values_df.index + 1
    # Find the ranks for the specified positions
    ranks = []
    if IS_DRUG_KEY:
        for row_idx, col_idx in positions_dict.items():
            specific_position = (df.index.get_loc(row_idx), df.columns.get_loc(col_idx))
            specific_rank = p_values_df[p_values_df['original_position'] == specific_position]['rank'].values[0]
            ranks.append(specific_rank)
    else:
        for col_idx, row_idx in positions_dict.items():
            specific_position = (df.index.get_loc(row_idx), df.columns.get_loc(col_idx))
            specific_rank = p_values_df[p_values_df['original_position'] == specific_position]['rank'].values[0]
            ranks.append(specific_rank)
    return ranks
...
def select_random_columns(df, num_sets, num_columns, max_zero_values):
    sets_of_columns = []
    # Remove NaNs
    df = df.dropna(axis=1, how='any')

    while len(sets_of_columns) < num_sets:
        # Randomly select columns
        selected_columns = random.sample(list(df.columns), num_columns)
        selected_df = df[selected_columns]

        # Check for sparsity (ensure each column has at most max_zero_values zeros)
        zero_counts_per_column = (selected_df == 0).sum(axis=0)

        if zero_counts_per_column.max() <= max_zero_values:
            sets_of_columns.append(selected_columns)

    return sets_of_columns
