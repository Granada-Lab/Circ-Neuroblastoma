########################################################################################################################
# SCRIPT FOR ANALYSIS PIPELINE, COLLABORATION WITH CHARITE-BERLIN, NEUROBLASTOMA PROJECT# March 2025, update June 2025 #
########################################################################################################################

# sysbio.epigen Group, DLSM, FSTM, University of Luxembourg
# PhD candidate Jeff Didier
# Dr Sebastien De Landtsheer
# Prof. Dr-Ing Thomas Sauter

# Running time cold: ~ 2 hours

# Update for revision 1 on 30.05.2025: figure_3_c_revised, figure_3_f_revised (increase running time from 10 to 90 mins)
# Update for revision 1 on 11.06.2025: figures with prefix 'update_' showing ALK-related gene analysis (20 mins added)
...
# Update: Gene list ALK pathway (https://www.nature.com/articles/s41698-023-00449-x, Fig. 1b)
# alk_pathway_genes = ['ALK', 'PIK3CA', 'AKT1', 'MTOR', 'MEK', 'ERK', 'RAF', 'NF1', 'STAT3', 'JAK1', 'JAK2', 'PLCγ']

# Following genes have different names:
# ['MEK', 'ERK', 'RAF', 'PLCγ']: MEK (MAP2K1, MAP2K2), ERK (MAPK1), RAF (RAF1), PLCγ (PLCG1, PLCG2)
alk_pathway_genes = ['ALK',
                     'PIK3CA', 'AKT1', 'MTOR',  # (RAS/RAF/MEK/ERK pathway)
                     'MAP2K1', 'MAP2K2', 'MAPK1', 'RAF1', 'NF1',  # (RAS/RAF/MEK/ERK pathway)
                     'STAT3', 'JAK1', 'JAK2',  # (JAK/STAT pathway)
                     'PLCG1', 'PLCG2']  # (Phosphoinositide signaling pathway)
...
# Update: Filter alk pathway genes
alk_expression = data_exp_norm[alk_pathway_genes]
...
# Update: Correlation analysis, spearman between ALK-related genes and drug sensitivity
for measure in drug_params:
    # Actual computation
    drug_meas_here = drug_data[['Primary Cell Line Name', 'Compound', measure]].set_index(['Primary Cell Line Name'])
    corr_matrix_here = pd.DataFrame(index=drug_availability_dict[measure], columns=alk_pathway_genes)
    pval_matrix_here = pd.DataFrame(index=drug_availability_dict[measure], columns=alk_pathway_genes)
    for drug in corr_matrix_here.index:
        this_drug_here = drug_meas_here.loc[drug_meas_here['Compound'] == drug][measure]
        for gene in alk_pathway_genes:
            expression_here = alk_expression.loc[remaining_celllines, gene]
            corr_matrix_here.loc[drug, gene] = spearmanr(this_drug_here, expression_here).statistic
            pval_matrix_here.loc[drug, gene] = spearmanr(this_drug_here, expression_here).pvalue
    # Plot
    fig = sns.clustermap(corr_matrix_here.astype(float), cmap=plt.get_cmap('coolwarm'),
    ...
    fig.ax_heatmap.axvline(x=0, color='k', linewidth=2)
    fig.ax_heatmap.axvline(x=corr_matrix_here.shape[1], color='k', linewidth=2)
    bonferroni_here = 0.05 / (len(drug_availability_dict[measure]) * len(alk_pathway_genes))
    fig.ax_col_dendrogram.set_title(
        f"Relationship between ALK-related gene expression and drug sensitivity\n"
        f"({measure.split()[0]}, Spearman r, {corr_matrix_here.shape[0]} drugs, "
        f"Bonferroni corrected p-value threshold = %.2e)" % bonferroni_here)
    correct_x = [x.get_text() for x in fig.ax_heatmap.get_xticklabels()]
    correct_y = [y.get_text() for y in fig.ax_heatmap.get_yticklabels()]
    ...
    cbar.ax.set_yticks([-1, 0, 1], [-1, 0, 1])
    # Save figure, correlations, and p values
    output_file = f'./{res_folder}/update_correlation_and_p_values_actarea_ALK.xlsx' if measure.split(' ')[0] == 'ActArea' else \
        f'./{res_folder}/update_correlation_and_p_values_ic50_ALK.xlsx' if measure.split(' ')[0] == 'IC50' else \
        f'./{res_folder}/update_correlation_and_p_values_ec50_ALK.xlsx'
    with pd.ExcelWriter(output_file) as writer:
        corr_matrix_here_reordered.to_excel(writer, sheet_name='correlation', index=False)
        pval_matrix_here_reordered.to_excel(writer, sheet_name='p-values', index=False)
    for form in formats:
        plt.savefig(f'./{res_folder}/update_correlation_actarea_ALK.{form}' if measure.split(' ')[0] == 'ActArea' else
                    f'./{res_folder}/update_correlation_ic50_ALK.{form}' if measure.split(' ')[0] == 'IC50' else
                    f'./{res_folder}/update_correlation_ec50_ALK.{form}', bbox_inches='tight', dpi=dpi)
    plt.close()

# Update: Linear regression of the configuration that throw the most significant hits with ALK genes [ActArea, Spearman]
# - MAP2K1-Topotecan, PIK3CA-PD0325901, PIK3CA-AZD6244 (spearman, 3 hits)

# In IC50: PIK3CA-Panobinostat, MAPK1-Topotecan, AKT1-TAE684
# In EC50: STAT3-Topotecan

# - SPEARMAN hits
measure = 'ActArea'
combs_of_interest = {'Topotecan': 'MAP2K1', 'PD0325901': 'PIK3CA', 'AZD6244': 'PIK3CA'}  # Drug : Gene

for drug, gene in combs_of_interest.items():
    drug_meas_here = drug_data[['Primary Cell Line Name', 'Compound', measure]].set_index(['Primary Cell Line Name'])
    this_drug_here = drug_meas_here.loc[drug_meas_here['Compound'] == drug][measure]
    this_drug_here.name = drug
    expression_here = alk_expression.loc[remaining_celllines, gene]
    ...
    # Annotate stats
    stats = spearmanr(df_reg[drug], df_reg[gene])
    bonferroni_threshold = 0.05/(20 * len(alk_pathway_genes))  # 20*14 we know this for act area
    plt.figtext(0.84, 0.62, f"R² = %.1f | r = %.1f\nBf$_{{adj ρ}}$ = %.1e" %
                (np.sqrt(np.abs(stats.statistic)), stats.statistic, stats.pvalue),
                ha="left", va="center", fontsize=10)
    plt.xlabel(f'{gene} gene expression')
    ...
    # Save figure
    for form in formats:
        plt.savefig(f'./{res_folder}/update_linear_regression_ALK_{drug}.{form}', bbox_inches='tight', dpi=dpi)
    plt.close()
...
# Storing the LOOCV in detail for figure S4b and S4d afterwards
...
# Also storing all LOOCV accuracies of every combination to colorize figure 3c and 3f by LOOCV performance
loocv_meta_circ_all = pd.DataFrame(index=['accuracy'])
loocv_meta_neuro_all = pd.DataFrame(index=['accuracy'])

# Scatter plot and bar plot ranking on drug-by-drug basis, colored by LOOCV LDA average accuracy
for comb, inp, target in zip(combination, inputs, targets):
    ...
    for g in gene_combos:
        ...
        # Get all single loocv performance
        print(f'LOOCV LDA of combination #{start} of {number_combos}.')
        start += 1
        loocv_acc, _, _, _, _, _, _ = LDA_loocv(data=expression_tmp,
                                                y=pd.concat([expression_tmp, this_score_binarized], axis=1),
                                                target='mra_circadian')
        if comb == 'clock_score':
            loocv_meta_circ_all[g] = loocv_acc * 100
        if comb == 'neuro_score':
            loocv_meta_neuro_all[g] = loocv_acc * 100
    # Plot all and highlight the best combination, and colorize by loocv performance
    scores_loocv = loocv_meta_circ_all.values if comb == 'clock_score' else loocv_meta_neuro_all.values
    # Set up normalization and colormap (green to red)
    norm = colors.Normalize(vmin=0.0, vmax=100.0)
    cmap = cm.get_cmap('RdYlGn')
    f, ax = plt.subplots()
    sc = ax.scatter(np.arange(len(gene_combos)), gene_combo_store.values, s=15 if comb == 'neuro_score' else 10,
                    c=scores_loocv, cmap=cmap, norm=norm, alpha=0.8 if comb == 'neuro_score' else 0.6, edgecolor='none')
    ...
    for gene_length in np.arange(1, len(combo_max_name) + 1):
        ...
        plt.scatter(xmax_here, ymax_here, marker='D', s=35, facecolors='none', edgecolors='black', linewidth=1.1)
    ...
    for form in formats:
        plt.savefig(f'./{res_folder}/Figure_3_c_revised.{form}' if comb == 'clock_score' else
                    f'./{res_folder}/Figure_3_f_revised.{form}', bbox_inches='tight', dpi=dpi)
    plt.close()
...
##############################################
# Update: LDA analysis on ALK-related gene set

# Step 1: Scatter plot and bar plot
expression_here = alk_expression.copy()
target_here = circ_score.set_index('Cellline').reindex(expression_here.index)
circ_score_here = target_here.loc[expression_here.index, 'mra_circadian']
this_score_binarized = circ_score_here >= circ_score_here.median()
subtypes_here = pd.DataFrame(list(circ_score_here.index), columns=['subtype'], index=circ_score_here.index)
loocv_acc, _, _, _, _, _, not_correct = \
    LDA_loocv(data=expression_here, y=pd.concat([expression_here, this_score_binarized], axis=1),
...
plot_LDA(expression_here, pd.concat([expression_here, this_score_binarized, subtypes_here], axis=1),
         label='mra_circadian',
         title=f'LDA, median-binarized, circ. score, ALK-related expression\n'
               f'ratio={"%.1f" % ratio}, LOOCV acc.={"%.2f" % loocv_acc}\n'
               f'Not correct: {list(not_correct)}',
         lda_output_target='mra_circadian', seed=seed, annot_subtype=True, annot_cellline=False,
         data_info=data_info, expand=(1.5, 2.2))
# Save figure
for form in formats:
    plt.savefig(f'./{res_folder}/update_LDA_scatter_ALK.{form}', bbox_inches='tight', dpi=dpi)
plt.close()

# STEP 2 ## analyse the best gene expression combinations, plot their bcd-wcd ratios and best LDA
# including their LOOCV accuracy.
cells = ['Accuracy (%)', 'SKNSH', 'GIMEN', 'NGP', 'SKNAS', 'KELLY', 'SHSY5Y', 'SKNBE2', 'CHP212']
top_genes_alk = [1, 2, 3, 4, 5, 6, len(alk_expression.columns)]

# Storing the LOOCV in detail
loocv_meta_alk = pd.DataFrame(index=cells, columns=top_genes_alk)
# Also storing all LOOCV accuracies of every combination to colorize by LOOCV performance
loocv_meta_alk_all = pd.DataFrame(index=['accuracy'])

# Scatter plot and bar plot ranking on drug-by-drug basis, colored by LOOCV LDA average accuracy

print(f'Processing LOOCV LDA in the ALK-related combination...')
expression_here = alk_expression.copy()
gene_combos = [comb for r in range(1, len(expression_here.columns) + 1) for comb in
               itertools.combinations(expression_here.columns, r)]
gene_combo_store = pd.DataFrame(index=['bcd-wcd_ratio'], columns=gene_combos)
...
for g in gene_combos:
    ...
    loocv_acc, _, _, _, _, _, _ = LDA_loocv(data=expression_tmp,
                                            y=pd.concat([expression_tmp, this_score_binarized], axis=1),
                                            target='mra_circadian')
    loocv_meta_alk_all[g] = loocv_acc * 100
# Plot all and highlight the best combination, and colorize by loocv performance
scores_loocv = loocv_meta_alk_all.values
# Set up normalization and colormap (green to red)
norm = colors.Normalize(vmin=0.0, vmax=100.0)
cmap = cm.get_cmap('RdYlGn')
f, ax = plt.subplots()
sc = ax.scatter(np.arange(len(gene_combos)), gene_combo_store.values, s=10,
                c=scores_loocv, cmap=cmap, norm=norm, alpha=0.8, edgecolor='none')
ax.axhline(y=0, color='grey', linestyle='--', linewidth=1)  # marking the zero line
ax.axhline(y=np.log10(2), color='grey', linestyle='-', linewidth=2)  # marking the ratio = 2 line
cbar = plt.colorbar(sc, ax=ax)
cbar.set_label('LOOCV Accuracy')
...
adjust_text(txts, expand=(0, 2.5), force_text=(1.5, 1.5), only_move={'text': 'y-'},
            arrowprops=dict(arrowstyle='-', color='k'))
plt.title(f'ALK ~ score ({len(expression_here.columns)} genes, {len(gene_combos)} combinations)')
plt.xlabel('# combination')
plt.ylabel('BCD-WCD ratio [log 10]')
for form in formats:
    plt.savefig(f'./{res_folder}/update_LDA_gene_combinations_ALK.{form}', bbox_inches='tight', dpi=dpi)
plt.close()
# Plot the best LDA with up to 6 genes, store also the 11/16 for the loocv meta plot
gene_length_to_plot = 6
for gene_length in np.append(np.arange(1, gene_length_to_plot + 1), len(expression_here.columns)):
    ...
    loocv_acc, _, _, _, _, _, not_correct = \
        LDA_loocv(data=expression_here[list(combo_max_here)],
                  y=pd.concat([expression_here[list(combo_max_here)], this_score_binarized], axis=1),
                  target='mra_circadian')
    loocv_meta_alk.loc['Accuracy (%)', gene_length] = loocv_acc * 100
    for cell in cells[1:]:
        loocv_meta_alk.loc[cell, gene_length] = 0 if cell in not_correct else 1
    if gene_length <= gene_length_to_plot:
        plot_LDA(expression_here[list(combo_max_here)],
                 pd.concat([expression_here[list(combo_max_here)], this_score_binarized, subtypes_here], axis=1),
                 label='mra_circadian',
                 ...
                 data_info=data_info, expand=(1.5, 2.2))
        for form in formats:
            plt.savefig(f'./{res_folder}/update_ALK_top_{gene_length}.{form}', bbox_inches='tight', dpi=dpi)
        plt.close()
print(f'Done with the ALK-related combination.\n')

# plot the loocv meta heatmap
mask = np.zeros_like(loocv_meta_alk, dtype=bool)
mask[0, :] = True
cmap = sns.color_palette(["lightgrey", "cornflowerblue"])
fig = sns.heatmap(loocv_meta_alk.astype(float), mask=mask, cmap=cmap, cbar=False, linewidths=0.5, linecolor='black',
                  annot=False, fmt="", annot_kws={"fontsize": 12, "color": "black"})
for i in range(loocv_meta_alk.shape[1]):
    fig.text(i + 0.5, 0.5, f'{loocv_meta_alk.iloc[0, i]:.1f}',
             ha='center', va='center', color='black', fontsize=8)
fig.xaxis.tick_top()
fig.xaxis.set_label_position('top')
plt.title('LOOCV - ALK-related genes')
plt.tight_layout()
for form in formats:
    plt.savefig(f'./{res_folder}/update_loocv_meta_heatmap_ALK.{form}', bbox_inches='tight', dpi=dpi)
plt.close()
