function cwt2_extract_ridge(folderPath_global,folderPath_adaptive,AllFiles_global,AllFiles_adaptive,is_bmal,is_per,celllinenames_file,reporters,ClusteringResults_FullDataSet)
...
disp('cwt2_extract_ridge.m is executed')
...
for un = 1:2 %loop 'un' unnormalized & normalized data
    ...
    for a = 1:numel(reporters) %loop a Luc-reporters
        ...
        ridge_length = nan(rows, numel(celllinenames_file));

        % Filter and sort data for reporter group
        if a == 1
            filtered_files = AllFileNames(:,is_bmal);
            %clusters = ClusteringResultsBmal1;
        elseif a == 2
            filtered_files = AllFileNames(:,is_per);
            %clusters = ClusteringResultsPer2;
        end

        clusters = ClusteringResults_FullDataSet;
        ...
        for c = 1:length(celllinenames_file) %loop c celllines
            ...
            if strcmp(celllinenames_file{c}, 'SKNBE')
                matches = matches & ~contains(filtered_files, 'SKNBE2');
            end
            ...
            for r = 1:numel(matchingFiles) %loop r replicates
                ...
                if un == 1
                    sampleName = regexprep(matchingFiles{r}, '^Ridge_adaptive_th_|\.csv$', '');
                else
                    sampleName = regexprep(matchingFiles{r}, '^Ridge_global_th_|\.csv$', '');
                end

                rowIdx = find(strcmp(clusters.Experiment, sampleName));
                MRAcluster = NaN;
                MRAcluster = clusters.Cluster(rowIdx);
                clear rowIdx

                if any(MRAcluster == 1) || any(MRAcluster == 2)
                    ...
                    %exclude ridges detected after 36 hours
                    if isempty(pyboatdata.time)
                    elseif pyboatdata.time(1) > 36
                        pyboatdata(:,:) =[];
                    end
                    ...
                    if TF == 0 %sudden jump identified.
                        ...
                        vars = {'lengthridges','ridgestartindices','ridgestartpoints','withinFirstXHours','longestRidgeIdx','longestRidgeStartIndex'};
                        clear(vars{:})
                ...
                vars2 = {'pyboatdata','pyboatdata_clean','ipt'};
                clear(vars2{:})
        ...
        circadian_parameters_excel = append('Results/extracted_circadian_parameters_by_replicate_',reporters{a},'.xlsx');
        ...
        vars3 = {'ridge_length','filtered_files','clusters'};
        clear(vars3{:})
...
disp('cwt2_extract_ridge.m is completed')
