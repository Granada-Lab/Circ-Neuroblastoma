...
Data = pd.read_excel("./Data/raw_lumicycle_data.xlsx") #,sep=",", skipinitialspace=True) %first few hours already excluded by hand, truncated to common recording length
#print (Data.columns)

c_label  = ["Noise", "Ultradian", "Circadian", "Infradian"]

workbook=xlsxwriter.Workbook('Data/processed_lumicycle_data.xlsx')
worksheet_mra=workbook.add_worksheet('mra')

#create folders to store ridge readout, wavelet spectra plots, and MRA plots.
try:
    os.mkdir('Figures/wavelet_plots')
    os.mkdir('Data/cwt_ridge_readout_normalized_detrended')
    os.mkdir('Data/cwt_ridge_readout_unnormalized_detrended')
    os.mkdir('Figures/multiresolution_analysis')
except FileExistsError:
    print(f"Folder 'Figures/wavelet_plots' already exists.")
    print(f"Folder 'Data/cwt_ridge_readout_normalized_detrended' already exists.")
    print(f"Folder 'Data/cwt_ridge_readout_unnormalized_detrended' already exists.")
    print(f"Folder 'Figures/multiresolution_analysis' already exists.")
...
#for ii in range(3): #to test out something new 
    ...
    global_th = 20 #don't differentiate in this manuscript between global and adaptive
    adaptive_th = 20
    ...
    ridge_results_adaptive_th.to_csv("./Data/cwt_ridge_readout_unnormalized_detrended/Ridge_adaptive_th_"+str(BiolData.columns[ii])+".csv") 
    ridge_results_global_th.to_csv("./Data/cwt_ridge_readout_normalized_detrended/Ridge_global_th_"+str(BiolData.columns[ii])+".csv")        
    ...
    fig4.savefig("Figures/wavelet_plots/wavelet_spectrum_normalized_detrended_adaptive_th_"+str(BiolData.columns[ii])+".svg")
    ...
    plt.savefig("./Figures/multiresolution_analysis/MRA_"+str(BiolData.columns[ii])+".png") #breast, U2OS, neuroblastoma
